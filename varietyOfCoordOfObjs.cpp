...
入口参数：
	...
	trackMode: 追踪算法的选择
	isImg: 若输入为图片，前提是图片已经保存在对应得文件夹中
出口参数：
	variety: 得到每个物体的xyz的变化。
			 这是二维vector，第一维表示物体，第二维表示图片
	objs: 记录所有检测到的物体
*/
void getVarietyOfCoordOfObjs(string videoFil, string trackMode, bool isImg,
	...
	vector<string> &objs) 
{
	...
	if (firstRunFlag == 1) { //如果是第一次运行
		...
		//更新firstRun.yml文件（修改firstRunFlag）
		firstRunfs.release();
		firstRunfs = FileStorage("firstRun.yml", FileStorage::WRITE);
		firstRunfs << "boardWidth" << boardWidth;
		firstRunfs << "boardHeight" << boardHeight;
		firstRunfs << "squareWidth" << squareWidth;
		firstRunfs << "squareHeight" << squareHeight;
		firstRunfs << "frameNumber" << frameNumber;
		firstRunfs << "firstRunFlag" << 0;
		firstRunfs.release();
	...
	if (isImg) {
		/*START 若既没有相机也没有视频，则从文件夹中取10个图片进行轨迹检测--------------------------------------*/
		for (int i = 0; i < frameNum; ++i) {
			/*生成图片路径*/
			string imgLPath = string("realTimeImg\\left\\");
			string imgRPath = string("realTimeImg\\right\\");
			string str1;
			stringstream stm;
			stm << i;
			stm >> str1;
			imgLPath = imgLPath + str1 + ".jpg";
			imgRPath = imgRPath + str1 + ".jpg";

			frameL = imread(imgLPath);
			frameR = imread(imgRPath);

			rgbImgLVec.push_back(imgLPath);
			rgbImgRVec.push_back(imgRPath);
		}
		...
		/*END 若既没有相机也没有视频，则从文件夹中取10个图片进行轨迹检测--------------------------------------*/
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	videoFile = fs["videoFile"];

	getVarietyOfCoordOfObjs(videoFile, "KCF", false, variety, objs);

	/*将物体追踪的结果存放在文件中*/
	ofstream fout("trackResults.txt");
	for (int i = 0; i < variety.size(); ++i) {
		fout << objs.at(i) << "\n";
		vector<pair<int, Vec3f>> objTrack = variety.at(i);
		for (int j = 0; j < objTrack.size(); ++j) {
			Vec3f v = objTrack.at(j).second;
			if (v[0] < 0) {
				float x = (float)(-v[0]);
				x = x / 1000.;
				fout << "左:" << setprecision(2) << x <<"米，"<< "前:" << setprecision(2) << v[2]/1000.<<"米";
			}
			else {
				float x = float(v[0]);
				x = x / 1000.;
				fout << "右:" << setprecision(2) << x <<"米,"<< "前:" << setprecision(2) << v[2]/1000.<<"米";
			}
			fout << "\n";
		}
	}
